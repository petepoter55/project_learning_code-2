...
public class HistoryOrderDtoRequestTest {
    private final static Logger logger = Logger.getLogger(HistoryOrderDtoRequestTest.class);
    ...
    public void validateInputRequest() throws Exception {
        ...
        try {
            for (File file : files){
                HistoryOrderDtoRequest historyOrderDtoRequest = mapper.readValue(file, HistoryOrderDtoRequest.class);
                List<String> actual = new ArrayList<>();

                Set<ConstraintViolation<HistoryOrderDtoRequest>> errors = validator.validate(historyOrderDtoRequest);
                for (ConstraintViolation<?> error : errors) {
                    actual.add(error.getPropertyPath().toString() + ": " + error.getMessage());
                }

                List<String> expected = mapper.readValue(FileUtils.readFileToString(new File(FilenameUtils.concat("./resources/products/case_validation/expected/", file.getName())), StandardCharsets.UTF_8), new TypeReference<List<String>>() {});
    ...
    private File[] readTestCase() throws Exception {
        File folder = new File("./resources/products/case_validation/request/");
